...
const itemTableWrapEl = document.getElementById('itemTableWrap');
const itemEmptyStateEl = document.getElementById('itemEmptyState');
...
function renderTable() {
  ...
  // Empty state: guide the person to the "+ Add Item" flow instead of
  // showing a blank table with just column headers and nothing under them.
  const isEmpty = pending.length === 0;
  itemTableWrapEl.classList.toggle('hide', isEmpty);
  itemEmptyStateEl.classList.toggle('show', isEmpty);
...
loadItemOptions();
e. Its success is shown immediately and
//                        is never retroactively hidden by anything below.
//                        Also returns rowIds — the Smartsheet row IDs just
//                        created — which step 2 needs for the attachment.
// 2. generatePrintPdf — writes the "MRF Print" Google Sheet, exports the
